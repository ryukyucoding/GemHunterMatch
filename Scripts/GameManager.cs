...
namespace Match3
{
    ...
    public class GameManager : MonoBehaviour
    {
        ...
        private void OnDestroy()
        {
            ...
            {
                LevelData.Instance.OnTimeUp.RemoveAllListeners();
            }
        }
        ...
        public void StartLevel()
        {
            ...
            {
                Debug.Log($"[GameManager] OnAllGoalFinished 被觸發（達成所有目標）");
                Instance.Board.ToggleInput(false);
                Instance.Board.TriggerFinalStretch();
            };

            // 監聽時間用完事件（只使用時間制）
            LevelData.Instance.OnTimeUp.AddListener(() =>
            {
                Debug.LogWarning($"[GameManager] OnTimeUp 被觸發（時間用完）");
                Instance.Board.ToggleInput(false);
                Instance.Board.TriggerFinalStretch();
            });
        ...
        public void ComputeCamera()
        {
            ...
            Camera.main.transform.position = center + Vector3.back * 10.0f;
