...
namespace Match3
{
    ...
    public class Board : MonoBehaviour
    {
        ...
        // GemType 到 FoodType 的映射（可在 Inspector 中配置）
        [Header("🍕 寶石到食材映射")]
        [Tooltip("設定每種寶石對應的食材類型")]
        public FoodType gemType0Food = FoodType.Bread;     // 藍色寶石 (Blue)
        public FoodType gemType1Food = FoodType.Egg;       // 黃色寶石 (Yellow)
        public FoodType gemType2Food = FoodType.Cheese;    // 白色寶石 (White)
        public FoodType gemType3Food = FoodType.Steak;     // 紫色寶石 (Purple)
        public FoodType gemType4Food = FoodType.Lettuce;   // 綠色寶石 (Green)
        public FoodType gemType5Food = FoodType.Tomato;    // 紅色寶石 (Red)
        
        // 動態映射字典
        private Dictionary<int, FoodType> s_GemToFoodMapping;
        ...
        void Awake()
        {
            s_Instance = this;
            GetReference();
            InitializeFoodMapping();

            // 確保在場景開始時重置失敗標誌
            m_FinalStretch = false;
            Debug.Log($"[Board] Awake - 重置 m_FinalStretch 為 false");
        }
        
        /// <summary>
        /// 初始化寶石到食材的映射
        /// </summary>
        private void InitializeFoodMapping()
        {
            s_GemToFoodMapping = new Dictionary<int, FoodType>
            {
                { 0, gemType0Food },
                { 1, gemType1Food },
                { 2, gemType2Food },
        ...
        {
            Debug.LogError($"[Board] ========== TriggerFinalStretch 被調用！==========");
            Debug.LogError($"當前時間: {LevelData.Instance?.RemainingTime:F1}秒");
            Debug.LogError($"呼叫堆疊：\n{System.Environment.StackTrace}");
            m_FinalStretch = true;
        }

        public void ResetFinalStretch()
        {
            m_FinalStretch = false;
            Debug.Log($"[Board] ResetFinalStretch 被調用，重置失敗標誌");
        }
        ...
        private void Update()
        {
            ...
            if (incrementHintTimer)
            {
                ...
                if (m_FinalStretch)
                {
                    Debug.LogError($"[Board] m_FinalStretch 為 true！準備顯示結束畫面。時間: {LevelData.Instance?.RemainingTime:F1}秒");
            ...
            for (int i = 0; i < m_TickingCells.Count; i++)
            {
                ...
                else if (currentCell.ContainingGem?.CurrentState == Gem.State.Bouncing)
                {
                    ...
                    if (gem.FallTime >= maxTime)
                    {
                        ...
                        gem.transform.localScale = gem.OriginalScale;
                        gem.StopBouncing();
                    ...
                    else
                    {
                        ...
                        var squishFactor = m_VisualSettingReference.SquishCurve.Evaluate(gem.FallTime);
                        gem.transform.localScale =
                            new Vector3(gem.OriginalScale.x, gem.OriginalScale.y * squishFactor, gem.OriginalScale.z);
                    }
                }
                ...
                for(int j = 0; j < match.MatchingGem.Count; j++)
                {
                    ...
                    if (match.ForcedDeletion || match.DeletionTimer > 1.0f)
                    {
                        ...
                        if (gem.CurrentState != Gem.State.Disappearing)
                        {
                            ...
                            // 料理系統整合：收集食材
                            if (s_GemToFoodMapping.ContainsKey(gem.GemType) && OrderManager.Instance != null)
                            {
                                FoodType foodType = s_GemToFoodMapping[gem.GemType];
                                Vector3 worldPosition = m_Grid.GetCellCenterWorld(gem.CurrentIndex);
                                OrderManager.Instance.CollectIngredient(foodType, 1, worldPosition);
                            }
